...
class PHPBrowserMobProxy_Client {
  ...
  function new_har($label = '', $options = array()) {
	$options['initialPageRef'] = $label;
	$data = http_build_query($options);

	$response = Requests::put("http://" . $this->browsermob_url . "/proxy/" . $this->port . "/har?$data",
							  array(),
							  array());
	return $response;
  }
  ...
  function new_page($label = '') {
	$data = "?pageRef=" . $label;
	$response = Requests::put("http://" . $this->browsermob_url . "/proxy/" . $this->port . "/har/pageRef$data",
							  array(),
							  array());
	return $response;
  }
